 ...
 *
 * Frogger is a four-way game: one hop per direction, and never two at once.
 *
 *   twist left / right  -> left and right
 *   tip away / toward   -> up and down
 *   BOOT button         -> hop forward; hold 3 s for sound off and on
...
#define HOP_DEG               10.0f    /* tilt this far to move left or right */
...
void input_update(fr_input_t *in)
{
    ...
    in->up     = ((dir_y > 0) || st.boot) ? 1 : 0;    /* the button hops forward */
    in->down   = (dir_y < 0) ? 1 : 0;
