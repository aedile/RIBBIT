 ...
 *
 * Frogger is a strict four-way game: one hop per push, and the stick has to come back to centre
 * before the next one counts. Tilt suits that well, because letting go returns it to centre for
 * you. Only the dominant axis is reported, so a diagonal tilt never produces two directions.
 *
 *   tilt left / right   -> hop left / right, the direction of gravity within the panel plane
 *   tilt away / toward  -> hop forward / back, how far gravity leaves that plane
 *   BOOT button         -> hop forward, since that is most of the game and the button would
 *                          otherwise do nothing on this one
...
#define HOP_DEG        10.0f    /* tilt this far to register a hop */
...
void input_update(fr_input_t *in)
{
    ...
    if (imu_ok && now - imu_last_us >= IMU_PERIOD_US) {
        ...
        if (fabsf(roll) >= fabsf(pitch)) {
            if (roll > HOP_DEG) dir_x = +1; else if (roll < -HOP_DEG) dir_x = -1;
        } else {
            if (pitch > HOP_DEG) dir_y = +1; else if (pitch < -HOP_DEG) dir_y = -1;
        }
    }

    in->left  = (dir_x < 0) ? 1 : 0;
    in->right = (dir_x > 0) ? 1 : 0;
    in->up    = ((dir_y > 0) || boot) ? 1 : 0;    /* the button hops forward */
    in->down  = (dir_y < 0) ? 1 : 0;

    if (now - last_log >= 3000000) {
        last_log = now;
        ESP_LOGI(TAG, "imu %d  roll %+6.1f pitch %+6.1f -> U%d D%d L%d R%d",
                 imu_ok, (double)dbg_roll, (double)dbg_pitch, in->up, in->down, in->left, in->right);
    }
}
