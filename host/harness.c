 ...
 * script keys: coin start up down left right
 */
#include <stdio.h>
#include <stdlib.h>
#include <string.h>
...
static void write_ppm(const char *path, const uint8_t *fb, const uint16_t *pal)
{
    ...
    if (!f) return;
    fprintf(f, "P6\n%d %d\n255\n", FR_FB_W, FR_FB_H);
    for (int y = 0; y < FR_FB_H; y++) {
        for (int x = 0; x < FR_FB_W; x++) {
            uint16_t c = pal[fb[y * FR_FB_W + x] % FR_PALETTE_SIZE];
...
int main(int argc, char **argv)
{
    ...
    int r10 = 0x00, r8 = 0x00;   /* 3 lives, 1 coin 1 play, upright */
    ...
    fr_roms_t roms = { fr_rom, fr_audiorom, fr_gfx, fr_prom };
    fr_init(&roms);
    fr_set_dips((uint8_t)r10, (uint8_t)r8);
    fr_input_t *in = fr_input();
    ...
    static uint8_t fb[FR_FB_W * FR_FB_H];
    static int16_t abuf[4096];
    uint16_t pal[FR_PALETTE_SIZE];
    ...
    for (int f = 0; f < frames; f++) {
        ...
        if (wav) {
            ...
            fr_render_audio(abuf, n, rate); fwrite(abuf, 2, n, wav); wav_samples += n;
        }
        if (now >= next_save) {
            char path[512]; snprintf(path, sizeof(path), "%s/frame_%03d.ppm", outdir, saved);
            fr_render(fb); fr_palette(pal); write_ppm(path, fb, pal); saved++; next_save += every;
        }
